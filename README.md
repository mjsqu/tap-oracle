...
```bash
pipx install tap-oracle
```

Install from GitHub:

```bash
pipx install git+https://github.com/ORG_NAME/tap-oracle.git@main
```
...
You can also test the `tap-oracle` CLI interface directly using `poetry run`:

```bash
poetry run tap-oracle --help
```
